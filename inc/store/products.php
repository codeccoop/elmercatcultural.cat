...
function emc_create_bound_product($post_id, $post_title)
{
	$product = new WC_Product_Simple();
	$product->set_name($post_title);
	$product->save();
	add_post_meta($product->id, 'event_id', (string) $post_id, true);
	return $product;
}
...
function emc_on_post_duplicated($original_id, $duplicate_id)
{
	...
	$product = emc_create_bound_product($duplicate_id, $post_title);
	...
	foreach ($EVENT_FIELDS_KEYMAP as $name => $index) {
		if (!isset($custom_fields[$name])) continue;
